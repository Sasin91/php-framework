...
class XmlVisitor extends AbstractRequestVisitor
{
    ...
    public function visit(CommandInterface $command, RequestInterface $request, Parameter $param, $value)
    {
        $xml = isset($this->data[$command])
            ? $this->data[$command]
            : $this->createROOT_PATHElement($param->getParent());
    ...
    public function after(CommandInterface $command, RequestInterface $request)
    {
        ...
        if (isset($this->data[$command])) {
            ...
            if ($operation->getData('xmlAllowEmpty')) {
                $xmlWriter = $this->createROOT_PATHElement($operation);
    ...
    /**
     * Create the ROOT_PATH XML element to use with a request
     *
     * @param Operation $operation Operation object
     *
     * @return \XMLWriter
     */
    protected function createROOT_PATHElement(Operation $operation)
    {
        static $defaultROOT_PATH = array('name' => 'Request');
        // If no ROOT_PATH element was specified, then just wrap the XML in 'Request'
        $ROOT_PATH = $operation->getData('xmlROOT_PATH') ?: $defaultROOT_PATH;
        ...
        $xmlWriter->startElement($ROOT_PATH['name']);
        // Create the wrapping element with no namespaces if no namespaces were present
        if (!empty($ROOT_PATH['namespaces'])) {
            // Create the wrapping element with an array of one or more namespaces
            foreach ((array) $ROOT_PATH['namespaces'] as $prefix => $uri) {
                $nsLabel = 'xmlns';
